...
class CrawlAHotel:
    ...
    @staticmethod
    def analyzeReviewPageModified(hotelid, newrevUrl, pagenum):
        ...
        """
        print 'in individual thread.'
        metaJson = []
        (fileContent, dwnld) = downloadToFile(baseUrl+newrevUrl, newrevUrl)
        if not dwnld:
            print 'error: downloading review url: ' + newrevUrl
            return metaJson
        contents = '\n'.join(fileContent)
        select = pq(contents)
        ...
        print 'dont with thread: ' + str(len(metaJson)) 
        return metaJson
    
    def cb_func(self, x):
        self.reviewJson.append(x)
    ...
    def getTAReviewsForHotel(self, revUrl, city):
        ...
        if not dwnld:
            print 'error: not able to parse the hotel url: ' + revUrl
            return {}
        fileContentStr = '\n'.join(fileContent)
        jsonObj2 = {}
        mincount = 0
        ...
        print 'done parsing hotel details.'
        ...
        print 'done downloading images details.'
        count = 0
        
        async_square = self.AsyncFactory(CrawlAHotel.analyzeReviewPageModified, self.cb_func)
        while count < totalpg:
            # create a url
            ...
            print 'spawning a new thread: ' + hotelid + ' : ' + newrevUrl + ' : ' + str(count)
            async_square.call(hotelid, newrevUrl,  count)
            ...
            if count == max_reviews:
                break
        print 'waiting for threads to get over!'
        async_square.wait()
        print 'threads have gotten over!'
        return {hotelid: {"details": jsonObj2, "reviews": self.reviewJson}}
...
def searchACity(city):
    ...
    hotelcount = 0
    resultJson = {}
    print 'Total Hotels: ' + str(len(hotelUrls))
...
#                successfulResults += 1
        ...
        crawlHotel = CrawlAHotel()
        hotelJson = crawlHotel.getTAReviewsForHotel(hUrl,city)
        totalReviews = 0
        if hotelJson != {}:
