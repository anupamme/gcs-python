...
def buildWordCloud(model):
    ...
    for attr in attributeMap:
        for subAttr in attributeMap[attr]:
            ...
            subAttributeCloud[attr][subAttr] = similarAttributes
            
    for attr in hierarchicalAttributeMap:
        similarAttributes = model.most_similar(attr)
        attributeCloud[attr] = similarAttributes
        ...
        if pos == 'NP' or pos == 'NN':
            ...
            for simAttr in similarAttributes:
                try:
                    ...
                    print ('Key Error: ' + str(n_word.encode('utf-8')))
...
def myfunc(name, line):
#        print ('type of args: ' + str(type(line)))
#        print("{} started!" + str(line) )
        findMostProbableAttributes(name, line)
#        output = '\n\"' + location + '\"' + '\t:\t' + json.dumps(resultJson)
#        outputFile.write(output)

def findMostProbableAttributes(name, inputLine):
    webSize = 5
    result = {}
    parts = inputLine.split('\t')
    locationKey = parts[0]
    hotelDetails = json.loads(parts[1])
    for hotelKey in hotelDetails:
        print ('hotelKey: ' + hotelKey)
        reviewSet = hotelDetails[hotelKey]
        reviewId = 0
        for review in reviewSet:        # format of reviewset :: [reviews]
            reviewDetails = []          #
            sentenceDetails = []
            for sentence in review:     # format of review :: [sentence]
                sentiment = sentence['sentiment']       #   sentiment :: sentence -> sentiment.
                wordDetails = sentence['worddetails']   #   worddetails :: array of dictionary.
                sentenceDetails.append((wordDetails, sentiment))    # json append.
                reviewDetails = reviewDetails + wordDetails         # array append.
...
def parseAndCreateSortedResults():
    ...
    global attributeCloud, subAttributeCloud
    attributeCloud, subAttributeCloud = buildWordCloud(model)
    inputDir = sys.argv[1]     # output of the stanford nlp routine.
    ...
    for fName in fileList:
        ...
        for line in lines:
            if len(line) < 10:
                continue
            #th = MyThread(name = count, args = tuple(line))
            th = threading.Thread(target = myfunc, args = (str(count), line))
            count += 1
            th.start()
    ...
    #sortedAttributesOnProbability = findMostProbableAttributes(inputData, model, attributeCloud, subAttributeCloud)
